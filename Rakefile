...
namespace :assets do
  ...
  task :compile_js do
    begin
      sprockets = MoscowYammerServer.settings.sprockets
      outpath   = MoscowYammerServer.settings.assets_path
      # 1 stage
  ...
  task :compile_html do
    begin
      ...
      # HTML
      outpath   = File.join(File.dirname(__FILE__), 'attachments')
      html_compressor = HtmlCompressor::HtmlCompressor.new
      # html files
      %w(index.html).each do |filename|
